...
import { ALL_MODULES, MODULE_LABELS, type PermissionModule, type UserPermission } from '@/types/database'
...
export default function PermissionsModal({
  ...
  async function handleSave() {
    ...
    try {
      const res = await fetch('/api/users/permissions', {
        ...
        body: JSON.stringify({ profileId, workspaceId, permissions }),
      })
  ...
  return (
    <div className="fixed inset-0 bg-dark-900/60 z-50 flex items-center justify-center p-4">
      <div className="bg-surface rounded-card shadow-card border border-border/50 w-full max-w-md">
        <div className="flex items-center justify-between px-6 pt-6 pb-4 border-b border-border/50">
          <div>
            <h3 className="font-heading font-semibold text-dark-700 text-lg">Permissões</h3>
            <p className="font-body text-xs text-dark-300 mt-0.5">{userName}</p>
          </div>
        ...
        <div className="px-6 py-4">
          ...
          {error && <p className="font-body text-sm text-danger mt-3">{error}</p>}
          {success && <p className="font-body text-sm text-success mt-3">{success}</p>}
        </div>
