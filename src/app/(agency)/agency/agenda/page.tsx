...
export default function AgencyAgendaPage() {
  ...
  const fetchEvents = useCallback(async (aid: string) => {
    ...
    try {
      ...
      console.error('Failed to fetch agency calendar events')
    } finally {
      setLoading(false)
    }
  }, [currentMonth])
  ...
  useEffect(() => {
    async function init() {
      const aid = await resolveAgency()
      if (aid) {
        setAgencyId(aid)
        await fetchEvents(aid)
      } else {
        setLoading(false)
      }
    }
    init()
  }, [resolveAgency, fetchEvents])
    ...
    return (
      <div className="space-y-6">
        <div className="flex items-end justify-between">
          <div>
            <h2 className="font-heading text-xl font-bold text-dark-700 mb-1">Agenda da Agência</h2>
            <p className="font-body text-sm text-dark-300">
              Organize a agenda interna da equipe.
            </p>
          </div>
          <button
        ...
        <EmptyState
          icon={CalendarDays}
          title="Nenhum evento da agência"
          description="Organize a agenda interna da equipe."
